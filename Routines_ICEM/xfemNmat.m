function [B] = xfemNmat(pt,e,type_elem,enrich_node,xCrl,GVertex,crack_node,cont,varargin)
...
if ( any(enrich_node(sctr(:)) == 1)) % B(x) enriched node
...
for in = 1:nn
  if ( enrich_node(sctr(in)) == 2)
    ...
    N_enr = [N(in)*(Hgp-Hi) 0 ; 0 N(in)*(Hgp-Hi)] ;
    Nxfem = [Nxfem N_enr];
    clear N_enr ;
