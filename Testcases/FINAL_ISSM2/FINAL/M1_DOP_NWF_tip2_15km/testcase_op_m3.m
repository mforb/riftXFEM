...
global quick_freeze
global fixwf
fixwf = 0 % this fixes the wall_force off (use with caution)
quick_freeze = 1
epsilon = 5 
...
%xCr(1).melange(1) = 0;
%xCr(1).melange(end) = 0;
xCr(1).width = [0 10 60 150 200 80 0 ] ;
results_path = './FINAL/M1_DOP_NWF_tip1_10km';
mkdir(results_path);
copyfile('testcase_op_m3.m',[results_path,'/']);
...
results_path = './FINAL/M1_DOP_NWF_tip1_20km';
mkdir(results_path);
copyfile('testcase_op_m3.m',[results_path,'/']);
run_mesh_prep
zoom_dim(1,:) = [min(xCr.coor(:,1))-20000,max(xCr.coor(:,1))+20000];
...
xCr(1).tip = [0,1];
results_path = './FINAL/M1_DOP_NWF_tip2_10km';
mkdir(results_path);
copyfile('testcase_op_m3.m',[results_path,'/']);
run_mesh_prep
zoom_dim(1,:) = [min(xCr.coor(:,1))-20000,max(xCr.coor(:,1))+20000];
...
results_path = './FINAL/M1_DOP_NWF_tip2_15km';
numstep = 4
mkdir(results_path);
copyfile('testcase_op_m3.m',[results_path,'/']);
run_mesh_prep
...
results_path = './FINAL/M1_DOP_NWF_tip3_5km';
mkdir(results_path);
copyfile('testcase_op_m3.m',[results_path,'/']);
run_mesh_prep
zoom_dim(1,:) = [min(xCr.coor(:,1))-20000,max(xCr.coor(:,1))+20000];
