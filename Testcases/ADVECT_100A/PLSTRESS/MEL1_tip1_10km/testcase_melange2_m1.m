...
%xCr(1).melange(1) = 0;
%xCr(1).melange(end) = 0;
%xCr(1).width = [min_gap 500 1000 1000 500 min_gap ] ;
%results_path = './PLSTRESS/MEL1_tip1_10km';
xCr(1).width = [min_gap 50 100 100 50 min_gap ] ;
results_path = './PLSTRESS/MEL1_tip1_10km';
mkdir(results_path);
copyfile('testcase_melange2_m1.m',[results_path,'/']);
...
%if( strcmp(plotmesh,'YES') )
    ...
    %end
%end
%results_path = './PLSTRESS/MEL1_tip1_20km';
%mkdir(results_path);
...
%end
%xCr(1).tip = [0,1];
%results_path = './PLSTRESS/MEL1_tip2_10km';
%mkdir(results_path);
%run_mesh_prep
%zoom_dim(1,:) = [min(xCr.coor(:,1))-20000,max(xCr.coor(:,1))+20000];
%zoom_dim(2,:) = [min(xCr.coor(:,2))-10000,max(xCr.coor(:,2))+10000];
...
%if( strcmp(plotmesh,'YES') )
    ...
    %end
%end
%results_path = './PLSTRESS/MEL1_tip2_15km';
%numstep = 4
%mkdir(results_path);
%copyfile('testcase_melange_m1.m',[results_path,'/']);
%run_mesh_prep
...
%if( strcmp(plotmesh,'YES') )
    ...
    %end
%end
%results_path = './PLSTRESS/MEL1_tip3_5km';
%mkdir(results_path);
%copyfile('testcase_melange_m1.m',[results_path,'/']);
%run_mesh_prep
%zoom_dim(1,:) = [min(xCr.coor(:,1))-20000,max(xCr.coor(:,1))+20000];
