...
numcrack = size(xCr,2) ;
xCr(1).coor = [-0.1 0.0844; 0.1 0.0844; 0.4  0.0844] ;
fmesh = figure();
...
close all
fmesh = figure();
xCr(1).coor = [-0.1 0.084764928;0.1 0.084764928; 0.4 0.084764928] ;
plotmesh = 'YES' ; plotNode = 'no' ;
if( strcmp(plotmesh,'YES') )
    plotMesh(node,element,elemType,'b-',plotNode,fmesh)
...
results_path = './Results2/Through';
mkdir(results_path);
[Knum2,ThetaInc2,xCr2] = mainXFEM(xCr,numstep,deltaInc) 

close all
fmesh = figure();
xCr(1).coor = [-0.1 0.08505;0.1 0.08505; 0.4 0.08505] ;
plotmesh = 'YES' ; plotNode = 'no' ;
if( strcmp(plotmesh,'YES') )
    plotMesh(node,element,elemType,'b-',plotNode,fmesh)
